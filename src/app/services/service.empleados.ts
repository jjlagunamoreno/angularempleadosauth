...
export class ServiceEmpleados {
    ...
    async loginEmpleado(user: Login): Promise<any> {
        ...
        try {
            ...
            this.token = response.data.response; // Guardamos el token al iniciar sesión
            return response.data.response; // Retornamos el token
        } catch (error) {
    ...
    async getPerfilEmpleado(): Promise<any> {
        ...
        try {
            const response = await axios.get(url, {
                headers: {
                    Authorization: `Bearer ${this.token}` // Configuramos la cabecera de autorización
                }
            });
